...
| 2023-05 | Voyager | Lifelong embodied agent loop | Minecraft: 3.3x more unique items, 2.3x longer travel, and up to 15.3x faster milestone unlocks versus prior SOTA. Cataloged in `data/techniques.yml` as "Skill Library / Lifelong Skill-Acquisition Loop". | [arXiv](https://arxiv.org/abs/2305.16291), [Project](https://voyager.minedojo.org/) |
| 2024-02 | CodeAct | Executable-code action loop | API-Bank and a curated benchmark across 17 LLMs; up to 20% higher success rate over common action formats. Cataloged in `data/techniques.yml` as "RLM / CodeAct REPL Loop". | [arXiv](https://arxiv.org/abs/2402.01030), [GitHub](https://github.com/xingyaoww/code-act) |
| 2022-03 | Self-Consistency | Test-time compute / decoding | GSM8K +17.9%, SVAMP +11.0%, AQuA +12.2%, StrategyQA +6.4%, ARC-challenge +3.9% over greedy chain-of-thought decoding, with no additional training. | [arXiv](https://arxiv.org/abs/2203.11171) |
| 2023-10 | Language Agent Tree Search (LATS) | Test-time compute / search loop | State-of-the-art 92.7% pass@1 on HumanEval with GPT-4; gradient-free WebShop performance comparable to fine-tuning. | [arXiv](https://arxiv.org/abs/2310.04406), [GitHub](https://github.com/lapisrocks/LanguageAgentTreeSearch) |
...
| 2024-06 | CRAG Benchmark | RAG evaluation benchmark | Shows advanced LLMs at <=34% and straightforward RAG around 44% accuracy on a dynamic factual QA benchmark. This is an evaluation benchmark, not a runtime technique, so it has no matching `data/techniques.yml` entry; it shares the "CRAG" acronym with, but is a different paper from, Corrective RAG above. | [arXiv](https://arxiv.org/abs/2406.04744), [GitHub](https://github.com/facebookresearch/CRAG/) |
| 2023-08 | Graph of Thoughts | Test-time compute / search loop | 62% higher sorting quality than Tree of Thoughts while reducing cost by more than 31%. | [arXiv](https://arxiv.org/abs/2308.09687), [GitHub](https://github.com/spcl/graph-of-thoughts) |
| 2023-05 | Verifier / PRM-Guided Step Search | Test-time compute / process supervision | Process-supervised reward model ("Let's Verify Step by Step") solves 78% of a representative MATH test subset, outperforming outcome supervision. | [arXiv](https://arxiv.org/abs/2305.20050) |
| 2023-05 / 2024-06 | Model Cascading / Tiered Routing | Cost runtime | FrugalGPT matches the best individual LLM with up to 98% cost reduction; RouteLLM reduces cost by over 2x on MMLU/MT-Bench while preserving quality. | [FrugalGPT arXiv](https://arxiv.org/abs/2305.05176), [RouteLLM arXiv](https://arxiv.org/abs/2406.18665) |
...
| 2026+ | Stateful harness around pure agent loop | Harness runtime | Strong implementation evidence from Tau: separates a stateless provider/tool loop from transcript ownership, queues, cancellation, event streaming, and transcript repair. Benchmark evidence is indirect. | [Tau harness](https://github.com/huggingface/tau/blob/main/src/tau_agent/harness.py), [Tau loop](https://github.com/huggingface/tau/blob/main/src/tau_agent/loop.py) |
| 2024-10 / 2025-06 | Swarm / Supervisor multi-agent architectures | Multi-agent handoff / centralized delegation | Public implementation evidence from OpenAI Swarm, LangGraph Swarm/Supervisor, and a 2025 LangChain benchmark over modified Tau-bench; results depend strongly on context/handoff details. Cataloged as two separate techniques: "Supervisor Architecture" and "Swarm Architecture". | [OpenAI Swarm](https://github.com/openai/swarm), [LangGraph Swarm](https://github.com/langchain-ai/langgraph-swarm-py), [Benchmark](https://www.langchain.com/blog/benchmarking-multi-agent-architectures) |
...
| 2025-03 | OpenAI Agents SDK primitives | Handoffs / guardrails / tracing / sessions | Production-oriented orchestration primitives; not a benchmark paper, but useful as runtime taxonomy evidence for the Guardrail Pipeline, Run Receipt, and HITL entries. | [OpenAI](https://openai.com/index/new-tools-for-building-agents/), [Docs](https://openai.github.io/openai-agents-python/) |
| 2024+ | LangGraph durable execution | Durable orchestration runtime | Durable execution, persistence, streaming, and human-in-the-loop are explicit runtime features; public benchmark evidence is indirect. Evidence source for both "Task Queue / Resumable Workflow Runtime" and "Workflow-Graph / State-Machine Runtime". | [Docs](https://docs.langchain.com/oss/python/langgraph/overview), [GitHub](https://github.com/langchain-ai/langgraph) |
| 2023-10 | MemGPT-Style Memory Paging | Memory runtime | Document analysis beyond the base LLM's context window and persona-consistent multi-session chat; evaluated on two focused domains rather than a broad public leaderboard. | [arXiv](https://arxiv.org/abs/2310.08560) |
...
| Published | Technique | Runtime Area | Benchmark / Evidence Signal | Sources |
| --- | --- | --- | --- | --- |
| 2025+ | Deep Research Loop | Research loop | Important product/runtime pattern; public evidence is product-level and benchmark details are not always reproducible. Cataloged in `data/techniques.yml` as "Deep Research Loop". | [OpenAI](https://openai.com/index/introducing-deep-research/) |
| 2026-07 | Onyx programs / heartbeat checkpoint loop | Programmable agent runtime | Strong conceptual runtime primitive: `run`, `spawn`, typed state, sleep, checkpoint, budget/error semantics. Public benchmark evidence not yet established. Cataloged as "Heartbeat / Checkpoint Loop", which this article's framing extends. | [X article](https://x.com/i/article/2073112080140689408) |
...
These were previously listed as needing more source work. They are now
cataloged in `data/techniques.yml`:

- Query routing and intent classification: see "Intent / Context / Routing
  Front Door".
- Long-context state summarization: see "Context Packing / Compression
  Loop" and "MemGPT-Style Memory Paging".
- Multi-agent communication protocols: see "MCP Tool / Context Protocol
  Layer" and "A2A / Agent Interoperability Protocol Layer".

## Newly Identified Gaps (2026-07-04 patch)

Techniques added in this patch that still need stronger source work before a
